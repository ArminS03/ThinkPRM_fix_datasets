...
class Config:
    ...
    gpu_memory_utilization: float = (
        0.98  # vllm is allocated 0.5 of GPU memory, the PRM uses the rest
    )
    prm_path: str = None
    ...
    cached_solutions_path: str = None
    ...
    prm_type: str = "thinkprm"
    prm_decision_temperature: float = 1.0
    prm_score_label_idx: int = 0
    prm_label_categories: str = "yes,no"
    prm_predecision_string: str = "\nIs the solution correct? "
    prm_verifier_instruction: str = None
    ...
    def __post_init__(self):
        ...
        if 'llama' not in self.model_path.lower():
            print(f"Warning: {self.model_path} is not a llama model, so custom chat template will not be used.")
            self.custom_chat_template = None # only llama models need custom chat template
