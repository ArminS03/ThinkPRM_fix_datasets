...
def compute_accuracy(dataset):
    ### will compare weighted_pred@1 with answer
    acc = 0
    for i in range(len(dataset)):
        canonical_answer = memoized_canonical_form(dataset[i]['answer'])
        weighted_pred = memoized_canonical_form(dataset[i]['pred_weighted@1'])
        if weighted_pred == canonical_answer:
            acc += 1
