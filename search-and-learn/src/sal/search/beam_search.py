...
def _beam_search(batch_of_prompts, config: Config, llm: LLM, prm: PRM) -> list[Beam]:
    ...
    for i in tqdm(range(config.num_iterations), desc="Beam search iterations"):
        ...
        for beam, gen_result in zip(active_beams, gen_results, strict=True):
            ...
            if config.truncate_beams:
                #### tokenize and truncate beam.current_text to max_tokens to avoid OOM with super long beams
                tokenized_text = tokenizer.encode(beam.current_text)
                if len(tokenized_text) > config.max_tokens:
                    beam.current_text = tokenizer.decode(tokenized_text[:config.max_tokens])
            completions.append([beam.current_text])

        scores = prm.score(prompts, completions, batch_size=config.prm_batch_size)
