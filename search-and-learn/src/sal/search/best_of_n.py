...
def best_of_n(x, config: Config, llm: LLM, prm: PRM):
    ...
    if config.n > 1 and not config.just_sample:   
        scores = prm.score(x["problem"], completions, batch_size=config.prm_batch_size)
        agg_scores = [
            [aggregate_scores(s, config.agg_strategy) for s in score] for score in scores
        ] # shape: (n_problems, n_completions)
    else:
        #### arbitrary scores, since this is greedy search
        scores = [
            [[1.0]] * len(completions[i]) for i in range(len(completions)) ]
        agg_scores = [
            [1.0] * len(completions[i]) for i in range(len(completions))
        ]
